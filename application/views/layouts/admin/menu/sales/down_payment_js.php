...
<script>
    $(document).ready(function () {
        console.log('FITUR: Update masih belum sesuai');
        // var csrfData = {};
        ...
        // $.alert('Update dan Cetak belum di buat');
        // Auto Set Navigation
        var identity = "<?php echo $identity; ?>";
        var menu_link = "<?php echo $_view; ?>";
        $(".nav-tabs").find('li[class="active"]').removeClass('active');
        $(".nav-tabs").find('li[data-name="finance/down_payment"]').addClass('active');
        ...
        new AutoNumeric('#cashback_debit', autoNumericOption2);
        ...
        */
        ...
        $(document).on("click", "#btn-save", function (e) {
            ...
            // }
            if (next == true) {
                var cashback_flag = $(".cashback_checkbox").attr('data-flag');
                if (parseInt(cashback_flag) == 0) {
                }else if (parseInt(cashback_flag) == 1) {
                    var debit = $("#cashback_debit").val();
                    var debit_account = $("#cashback_account").find(':selected').val();
                    if(removeCommas(debit) < 1){
                        notif(0,'Jumlah cashback wajib diisi');
                        next=false;
                        $("#cashback_debit").focus();
                    }

                    if(next){
                        if (parseInt(cashback_flag) == 0) {
                            notif(0,'Akun Cashback belum dipilih');
                            next = false;
                        }
                    }
                }
            ...
            if (next == true) {
                ...
                var prepare = {
                    ...
                    cashback_account: $("#cashback_account").find(":selected").val(),
                    cashback_value: $("#cashback_debit").val()
                }
        ...
        */
        ...
        function loadJournalItems(journal_ids = 0) {
            ...
            if (parseInt(journal_id) > 0) {
                var data = {
                    action: 'load-journal-items-down-payment',
                    tipe: identity,
                    journal_id: journal_id
                };
            } else {
                ...
                $.ajax({
                    ...
                    success: function (d) {
                        if (parseInt(d.status) === 1) { //Success
                            ...
                            if(parseInt(d.is_cashback) > 0){
                                checkBoxCashback(0);
                                $("#cashback_account").val(d.result_cashback.account_id).trigger('change');
                                $("#cashback_debit").val(d.result_cashback.journal_item_debit);
                            }
                        } else { //No Data
                            $("#table-item tbody").html('');
                            $("#table-item tbody").html('<tr><td colspan="4">- Tidak ada item -</td></tr>'); // 
        ...
        $(document).on("click", ".cashback_checkbox", function (e) {
            var check = $(".cashback_checkbox").attr('data-flag');
            if (check == 0) {
                checkBoxCashback(0);
            } else {
                checkBoxCashback(1);
    ...
    function checkBoxCashback(flag) {
        if (flag == 0) {
            $("#cashback_flag").prop("checked", true);
            $(".cashback_checkbox").attr("data-flag", 1);
            // formTransDepSetDisplay(1);
            $("#cashback_debit").attr('readonly',false);
            $("#cashback_account").removeAttr('disabled');
        } else {
            $("#cashback_flag").prop("checked", false);
            $(".cashback_checkbox").attr("data-flag", 0);
            $("#cashback_debit").attr('readonly',true);
            $("#cashback_debit").val(0);               
            $("#cashback_account").attr('disabled',true);
            $("#cashback_account").val(0).trigger('change');                     
            // formTransDepSetDisplay(0);
            // $("#asset-checkbox-label").html('Asset depresiasi');
        }
        var fl = $(".cashback_checkbox").attr('data-flag');   
    }    
</script>