...
<script>
    $(document).ready(function () {
        // $.alert('Sampai Dengan: btn-edit-item e_total, perhitungan rumus Jquery');
        // var konten = '';
        //     konten += '- Update order_total on create-item, delete-item, update-item<br>'+
        //               '';
        // $.confirm({
        //   title: 'Info',
        //   content: konten,
        ...
        $(".nav-tabs").find('li[data-name="sales/quotation"]').addClass('active');
        ...
        $(document).on("click", ".btn-check-item", function (e) {
            e.preventDefault();
            var id = $(this).attr("data-order-item-id");
            var flag = $(this).attr("data-order-item-flag");
            if (flag == 1) {
                var set_flag = 1;
                var msg = 'unchecklist';
            } else {
            ...
            $.confirm({
                title: 'Set Checklist',
                content: 'Apakah anda ingin <b>' + msg + '</b> <b>' + nama + '</b> ?',
                columnClass: 'col-md-4 col-md-offset-4 col-sm-6 col-sm-offset-3 col-xs-10 col-xs-offset-1',
                autoClose: 'button_2|10000',
                closeIcon: true,
                closeIconClass: 'fas fa-times',
                    ...
                    button_1: {
                        ...
                        action: function (e) {
                            ...
                            if (!input) {
                                ...
                                form.append('order_item_qty', removeCommas(input));
                                $.ajax({
                                    type: "post",
                                    url: url,
                    ...
                    if (parseInt(d.status) === 1) { //Success
                        ...
                        if (parseInt(total_records) > 0) {
                            ...
                            for (var a = 0; a < total_records; a++) {
                                ...
                                dsp += '<a href="#" class="btn-change-item-qty" data-order-item-id="' + d.result[a]['order_item_id'] + '" data-order-item-qty="' + d.result[a]['order_item_qty'] + '">' + d.result[a]['order_item_qty'] + ' ' + d.result[a]['order_item_unit'] + '</a>';
                                dsp += '</td>';
                                ...
                                if (d.result[a]['order_item_flag'] == 1) {
                                    dsp += '<button type="button" class="btn-check-item btn btn-mini btn-primary" data-order-id="' + d.result[a]['order_item_order_id'] + '" data-order-item-id="' + d.result[a]['order_item_id'] + '" data-order-item-flag="' + d.result[a]['order_item_flag'] + '" data-order-item-product-id="' + d.result[a]['product_id'] + '" data-nama="' + d.result[a]['product_name'] + '" data-kode="' + d.result[a]['product_code'] + '" data-order-item-unit="' + d.result[a]['order_item_unit'] + '" data-order-item-price="' + d.result[a]['order_item_price'] + '" data-order-item-qty="' + d.result[a]['order_item_qty'] + '" data-order-item-total="' + d.result[a]['order_item_total'] + '"data-order-item-note="' + d.result[a]['order_item_note'] + '">';
                                    dsp += '<span class="fas fa-check"></span>';
                                    dsp += ' Checked</button>';
                                } else if (d.result[a]['order_item_flag'] == 0) {
                                    dsp += '<button type="button" style="background-color:#f79407;color:white;" class="btn-check-item btn btn-mini" data-order-id="' + d.result[a]['order_item_order_id'] + '" data-order-item-id="' + d.result[a]['order_item_id'] + '" data-order-item-flag="' + d.result[a]['order_item_flag'] + '" data-order-item-product-id="' + d.result[a]['product_id'] + '" data-nama="' + d.result[a]['product_name'] + '" data-kode="' + d.result[a]['product_code'] + '" data-order-item-unit="' + d.result[a]['order_item_unit'] + '" data-order-item-price="' + d.result[a]['order_item_price'] + '" data-order-item-qty="' + d.result[a]['order_item_qty'] + '" data-order-item-total="' + d.result[a]['order_item_total'] + '"data-order-item-note="' + d.result[a]['order_item_note'] + '">';
                                    dsp += '<span class="fas fa-check-circle"></span>';
                                    dsp += ' Uncheck</button>';
                                }
                                dsp += '&nbsp;';
