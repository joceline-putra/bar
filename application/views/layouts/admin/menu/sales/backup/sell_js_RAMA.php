...
$(document).ready(function(){
  ...
  // $.alert('Prepare Penjualan Bengkel');
  // var konten = '';
  // konten += 
  ...
  $(function() {
    ...
    $("#start, #end").datepicker({
      ...
      todayHighlight: true,
      weekStart: 1 
    }).on('changeDate', function(){ index.ajax.reload(); }); 
  });
  ...
  var index = $("#table-data").DataTable({
    ...
    "columns": [{
        ...
        render: function(data, meta, row) {
          ...
          dsp += '&nbsp;<button class="btn-return btn btn-mini btn-default" data-trans-id="'+row.trans_id+'" data-contact-id="'+ row.contact_id +'">';
    ...
    $.ajax({
      ...
      success:function(d){
        if(parseInt(d.status)==1){ /* Success Message */
          ...
          $("#trans_vehicle_brand").val(d.result.trans_vehicle_brand).trigger('change');
          $("#trans_vehicle_brand_type_name").val(d.result.trans_vehicle_brand_type_name).trigger('change');
          $("#trans_vehicle_distance").val(d.result.trans_vehicle_distance);
  ...
  $(document).on("click",".btn-delete",function() {
    ...
    $.confirm({
      ...
      onContentReady: function(){
        this.setContentAppend('<div>Apakah anda ingin menghapus Penjualan ?<br>Nomor: <b>'+number+'</b><br>Tanggal: <b>'+tgl+'</b><br>Customer: <b>'+contact+'</b><br>Tagihan: <b>'+total+'</b></div>');
      },
      buttons: {
        button_1: {
          text:'Hapus',
                ...
                if(parseInt(d.status)==1){ 
                  ...
                  if(parseInt(d.status) == 4){
                    $.confirm({
                        ...
                        content:'Penjualan <b>'+number+'</b> sudah dilunasi menggunakan<br><br>',
                        onContentReady: function(){
                            var self = this;
                            var dsp = '';
  ...
  $(document).on("click",".btn-print-dropdown",function() {
    ...
    if(session.length > 0){
      ...
      if(action == 'print_struk'){
        ...
        var set_print_url = url_print+'_struk_bengkel/'+trans_id;
  ...
  $(document).on("click",".btn-print-whatsapp",function(e) {
    ...
    var trans_number = $(".modal-trans-number").attr('data-trans-number');
    var trans_date = $(".modal-trans-date").attr('data-trans-date');
    var trans_total = $(".modal-trans-total").attr('data-trans-total');

    var contact_name = $("#modal-contact-name").val();
    var contact_phone = $("#modal-contact-phone").val();
    ...
    $.confirm({
        ...
        onContentReady: function(e){
            ...
            dsp += '<form id="jc_form">';
                ...
                dsp += '         <option value="Coli">Coli</option>';
  ...
  $(document).on("input","#total_diskon",function(e) {
    ...
    result_2 = (parseFloat(subtotal)+parseFloat(ppn))-parseFloat(diskon);   
  ...
  $(document).on("input","#harga, #qty, #diskon_persen",function(e) {
    ...
    // var diskon = "0,00";
    // var diskon_persen = "0,00";

    var result = "0.00";
    var percent = "0.00";

    if(harga.length > 0){ harga = removeCommas(harga); }
    if(qty.length > 0){ qty = removeCommas(qty); }     
  ...
  $(document).on("click",".btn-pay-from-modal",function(e) {
    ...
    if(metode_pembayaran > 0){
      ...
      if(metode_pembayaran == 2){ //Transfer Bank
        ...
        modal_metode_pembayaran = 'Gratis & Tanggung Klinikita';
        next = true;
  ...
  $(document).on("click","#btn-approval",function(e) {
    ...
    if(parseInt(id) > 0){
      ...
      $.ajax({
        ...
        },error:function(xhr, Status, err){ notif(0,err); }
      });
      // alert(approval_flag);
      // if(next){
  ...
  */
  ...
  function test(){
    ...
    var t_number = 'Penjualan-2112-00002';
    var t_date = '11 Dec 2021';
    var t_total = 'Rp. 25.000,00';
    var c_name = 'Bpk. Habibah';
    var c_phone = '6281225518118';
