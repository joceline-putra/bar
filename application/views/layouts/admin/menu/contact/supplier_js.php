<script>
    $(document).ready(function () {
        ...
        // $("#img-preview1").attr('src', url_image);s
        // console.log("Identity:"+identity,"View:"+view);
        var alias = "<?= $title; ?>";

        $(".nav-tabs").find('li[class="active"]').removeClass('active');
        $(".nav-tabs").find('li[data-name="contact/supplier"]').addClass('active');
        ...
        new AutoNumeric('#contact_payable_limit', autoNumericOption);
        ...
        var index = $("#table-data").DataTable({
            ...
            "columns": [{
                    ...
                    render: function (data, meta, row) {
                        ...
                        if (parseFloat(row.contact_payable_balance) > 0) {
                            // dsp += '<br><label class="btn-receivable-payable label label-danger" style="cursor:pointer;" data-contact-id="' + row.contact_id + '" data-trans-type="1"><i class="fas fa-exclamation-triangle"></i> Sisa Hutang: ' + addCommas(row.contact_payable_balance) + "</label>";
                        }
                        if (parseFloat(row.contact_payable_limit) > 0) {
                            // dsp += '<label class="label label-info"><i class="fas fa-hand-holding-usd"></i> Limit Hutang: ' + addCommas(row.contact_payable_limit) + "</label>";
                        }
                        return dsp;
                    }
                }, {
        ...
        $(document).on("click", "#btn-save", function (e) {
            ...
            if (next == true) {
                ...
                form.append('contact_payable_limit', removeCommas($('#contact_payable_limit').val()));
            ...
            $.ajax({
                ...
                success: function (d) {
                    if (parseInt(d.status) == 1) { /* Success Message */
                        ...
                        $("#contact_payable_limit").val(d.result.contact_payable_limit);
        ...
        $(document).on("click", ".btn-receivable-payable", function (e) {
            ...
            $.confirm({
                ...
                content: function () {
                    var self = this;
                    var form = new FormData();
                    form.append('action', 'load-account-payable');
        ...
        */
        ...
        function formNew() {
            ...
            $("#btn-save").show();
            $("#btn-cancel").show();
            $("#contact_payable_limit").val('0.00');
        }
        function formCancel() {
            formMasterSetDisplay(1);
            $("#form-master input").val('');
