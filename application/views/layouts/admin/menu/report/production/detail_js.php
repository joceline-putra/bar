...
<script>
    $(document).ready(function () {
        ...
        $(".nav-tabs").find('li[data-name="report/production/product/detail"]').addClass('active');
        ...
        var index = $("#table-data").DataTable({
            ...
            "ajax": {
                ...
                cache: 'false',
                data: function (d) {
                    d.action = 'load-trans-items-for-report-production';
                    d.tipe = 8;
                    d.position = 1;
            ...
            "columnDefs": [
                ...
                {"targets": 2, "title": "Kode", "searchable": false, "orderable": false},
                {"targets": 3, "title": "Nama "+product_alias, "searchable": false, "orderable": false},
                {"targets": 4, "title": "Gudang", "searchable": false, "orderable": false},
            ...
            "columns": [{
                    ...
                    render: function (data, meta, row) {
                        var dsp = '';
                        dsp += addCommas(row.trans_item_in_qty) + ' ' + row.trans_item_unit;
                        return dsp;
                    }
                }, {
                    'data': 'trans_item_total',
                    className: 'text-right',
                    render: function (data, meta, row) {
                        var dsp = '';
                        dsp += addCommas(row.trans_item_total);
        ...
        /*
         else if(parseInt(identity) == 2){ // Purchase Order 
         var index = $("#table-data").DataTable({
         // "processing": true,
         "serverSide": true,
         "ajax": {
         ...
         else if(parseInt(identity) == 3){ // Penjualan
         var index = $("#table-data").DataTable({
         // "processing": true,
         "serverSide": true,
         "ajax": {
         ...
         'data': 'trans_paid_type',
         render:function(data,meta,row){
         var dsp = '';
         if(parseInt(row.trans_paid_type) == 1){ //Tunai
         dsp += 'Tunai';
         }else if(parseInt(row.trans_paid_type) == 2){ //Bank
         dsp += 'Bank';
         }else if(parseInt(row.trans_paid_type) == 3){ //Kredit
         dsp += 'Kartu Kredit: '+row.trans_card_bank_name;
         }else if(parseInt(row.trans_paid_type) == 4){ //Debit
         dsp += 'Kartu Debit: '+row.trans_card_bank_name;
         }else if(parseInt(row.trans_paid_type) == 5){ //Digital
         dsp += row.trans_digital_provider;              
         }else{
         dsp += '-';
         }                        
         return dsp;
         ...
         'data': 'trans_id', className: 'text-right',
         render:function(data,meta,row){
         var dsp = '';
         dsp += addCommas(row.trans_total);
         return dsp;
         ...
         var dsp = '';
         // dsp += row.order_id;
         dsp += '<button id="" class="btn btn-info btn-mini btn-print-penjualan" data-id="'+row.trans_id+'" data-number="'+row.trans_number+'">Print</button>';
         return dsp;
         }
         }]
         });
         } 
         else if(parseInt(identity) == 4){ // Pembelian
         var index = $("#table-data").DataTable({
         // "processing": true,
         "serverSide": true,
         "ajax": {
         ...
         'data': 'order_subtotal', className: 'text-right',
         render:function(data,meta,row){
         var dsp = '';
         ...
         'data': 'order_id', className: 'text-right',
         render:function(data,meta,row){
         var dsp = '';
         // dsp += row.order_id;
         // dsp += 'Print';
         dsp += '<button id="" class="btn btn-info btn-mini btn-print-pembelian" data-id="'+row.order_id+'" data-number="'+row.order_number+'">Print</button>';
         return dsp;
         }
         }]
         });
         }  
         else if(parseInt(identity) == 5){ // Laba Rugi
         var index = $("#table-data").DataTable({
         // "processing": true,
         "serverSide": true,
         "ajax": {
        ...
        $(document).on("change", "#filter_location, #filter_produk, #filter_order, #filter_dir", function (e) {
            index.ajax.reload();
        });
        ...
        $(document).on("click", ".btn-print-sales-order", function (e) {
            e.preventDefault();
            e.stopPropagation();
            // console.log($(this));
            ...
            window.open(url_print_sales_order + id, 'Sales Order', 'width=700,height=485,left=' + x + ',top=' + y + '').print();
        });
        ...
        $(document).on("click", ".btn-print-penjualan", function (e) {
            ...
            window.open(url_print_penjualan + id, 'Penjualan', 'width=700,height=485,left=' + x + ',top=' + y + '').print();
        });
        ...
        $(document).on("click", ".btn-print-pembelian", function (e) {
            ...
            window.open(url_print_pembelian + id, 'Pembelian', 'width=700,height=485,left=' + x + ',top=' + y + '').print();
        });
        /*
         $('#table-data').on('page.dt', function () {
         var info = index.page.info();
        ...
        /*$(document).on("click",".btn-print",function() {
         ...
         var x = screen.width / 2 - 700 / 2;
         var y = screen.height / 2 - 450 / 2;
         var print_url = url_print +'/'+ btoa(id);
         var win = window.open(print_url,'Print Laporan Penjualan','width=700,height=485,left=' + x + ',top=' + y + '').print();   
         });*/
        $(document).on("click", ".btn-print-all", function () {
            ...
            var print_url = url_print + '/'
                    ...
                    + $("#start").val() + '/'
                    + $("#end").val() + '/'
                    + location + "?product=" + product + "&format=" + format + "&order=" + order + "&dir=" + dir;
