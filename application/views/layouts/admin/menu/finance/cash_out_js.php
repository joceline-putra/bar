<script>
  $(document).ready(function () {
    ...
    $(".nav-tabs").find('li[data-name="finance/cash_out"]').addClass('active');
    ...
    */
    ...
    $(document).on("click","#btn-save-account",function(e) {
      ...
      if(next==true){
        if($("select[id='group-akun']").find(':selected').val() == 0){
          notif(0,'Group wajib diisi');
          next=false;
        }   
      }

      if(next==true){
        if($("select[id='group-sub-akun']").find(':selected').val() == 0){
          notif(0,'Group Sub wajib diisi');
          next=false;
        }   
      }
      ...
      if(next==true){
        var prepare = {
          ...
          group:$("#group-akun").val(),
          group_sub:$("#group-sub-akun").find(':selected').val()
        }
        var prepare_data = JSON.stringify(prepare);
        var data = {
          action: 'create-from-modal',
