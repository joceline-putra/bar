...
<script>
    $(document).ready(function () {
        // $.alert('Sampai Dengan: JS 116 Date');
        // var konten = '';
        ...
        $(".nav-tabs").find('li[data-name="purchase/quotation"]').addClass('active');
        ...
        /*
         function reloadDate(){
         var tgl = $("#tgl").val().split("-");
         var tgl_tempo = $("#tgl_tempo").val().split("-");
         var tgl_convert = tgl[2]+'-'+tgl[1]+'-'+tgl[0];
         var tgl_tempo_convert = tgl_tempo[2]+'-'+tgl_tempo[1]+'-'+tgl_tempo[0];  
         var diff = new Date(tgl_tempo_convert) - new Date(tgl_convert);
         var days = diff/1000/60/60/24;
         if(days == 15){
         $("#syarat_pembayaran").val(15).trigger('change');
         }else if(days == 30){
         $("#syarat_pembayaran").val(30).trigger('change');
         }else if(days == 60){
         $("#syarat_pembayaran").val(60).trigger('change');
         }else if(days == 90){
         $("#syarat_pembayaran").val(90).trigger('change');
         }else{
         $("#syarat_pembayaran").val(0).trigger('change');
         }
         }
         ...
         var tgl = $("#tgl").val().split("-");
         var tgl_convert = tgl[2]+'-'+tgl[1]+'-'+tgl[0];  
         var from_date = new Date(tgl_convert);
         var date1 = from_date.setDate(from_date.getDate() + 2);
         // console.log(to_date);
         var date = new Date(date1);
         // var ss = ((date.getMonth() > 8) ? (date.getMonth() + 1) : ('0' + (date.getMonth() + 1))) + '-' + ((date.getDate() > 9) ? date.getDate() : ('0' + date.getDate())) + '-' + date.getFullYear(); 
         var bln = ((date.getMonth() > 8) ? (date.getMonth() + 1) : ('0' + (date.getMonth() + 1)));
         var tgl = ((date.getDate() > 9) ? date.getDate() : ('0' + date.getDate()));
         var thn = date.getFullYear();
         var set_date = tgl+'-'+bln+'-'+thn;    
         // var ss = (((date.getDate() > 9) ? date.getDate() : ('0' + date.getDate())) + '-' + (date.getMonth() > 8) ? (date.getMonth() + 1) : ('0' + (date.getMonth() + 1))) + '-' + date.getFullYear();
         $("#tgl_tempo").datepicker("update", set_date);
         // $('#tgl_tempo').datepicker("setDate", new Date(2008,9,03));
         // var mydv = moment(set_date).format('dd-mm-yyyy');
         // $('#tgl_tempo').datepicker().datepicker('setDate',mydv);  
